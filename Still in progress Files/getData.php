...
var bool = false;
   ...
   <?php
   //    session_start();
   //    $_SESSION['course_info'] = 'batman';

   // Fetch records from database
   $result = $db->query(
       'SELECT long_name, short_name FROM cluster WHERE parent_clusterid = "1"'
       //    'SELECT c.long_name AS cluster_long_name, c.short_name AS cluster_short_name, c.id AS cluster_id, scu.long_name AS course_long_name, scu.short_name AS course_short_name from scu as scu, cluster c WHERE c.id = scu.clusterid'
   );
   if ($result->num_rows > 0) {
       while ($row = $result->fetch_assoc()) { ?>

        var name = <?php if ($row['short_name'] != null) {
            echo '"[' . $row['short_name'] . '] ' . $row['long_name'] . '"';
        } else {
            ...
            // echo '<script>console.log("' . $acronym . '");</script>';
            echo '"[' . $acronym . '] ' . $row['long_name'] . '"';
        } ?>;
        var li = document.createElement('li');
        var link = document.createElement('a');
        link.href = '#';
    ...
    <?php }
        ...
        ?>
        console.log('No member(s) found...');
    <?php
   }
   ?>

d3.json("data.php").then(function(error, data){
  console.log(data);
});
